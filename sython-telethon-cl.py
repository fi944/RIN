...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        buttons = [
            [Button.inline('< 𝙎𝙊𝙐𝙍𝘾𝙀 𝙍𝙄𝙉 𖠛 >', 'sython')],
...
async def handler(event):
    data = event.data.decode('utf-8')
    if data == 'sython':
        await bot.send_message(OWNER_ID, f"""**∘ بوت رين خاص لتجميع والاوامر الاخرى 

∘ مطور ومبرمج البوت رين - @Ridhad
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('رشق مشاهدات ⟐', 'akbo')],
         ...
         [Button.inline('༺ 𝙎𝙊𝙐𝙍𝘾𝙀 𝙍𝙄𝙉 ༻', 'button0')]
        ]
        await bot.send_message(chat, '''**[̲̅S̲̅].[̲̅O̲̅].[̲̅U̲̅].[̲̅R̲̅].[̲̅C̲̅].[̲̅E̲̅].  .[̲̅R̲̅].[̲̅I̲̅].[̲̅N̲̅].**''', buttons=buttons)
...
@bot.on(events.NewMessage(pattern='/python', from_users=6658615502))
...
# • [̲̅S̲̅].[̲̅O̲̅].[̲̅U̲̅].[̲̅R̲̅].[̲̅C̲̅].[̲̅E̲̅].  .[̲̅R̲̅].[̲̅I̲̅].[̲̅N̲̅]. • #
